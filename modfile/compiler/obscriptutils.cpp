...
bool ObCompareScriptLocalPriv (obscriptcompare_t& CompareInfo) {
  ...
  while (Result) {
    ...
    pLocal2 = CompareInfo.pScript2->FindVariable(pName1->GetString().c_str());

    if (pLocal2 == NULL) {
      SystemLog.Printf("\tScript1 local variable '%s' was not found in script2!", pName1->GetString().c_str());
    }
		/* Compare the local variable data */
    else {
      if (pLocal1->GetSlsdData().Index != pLocal2->GetSlsdData().Index) {
        SystemLog.Printf("\tScript local variable '%s' indexes do not match: %u != %u", pName1->GetString().c_str(), pLocal1->GetSlsdData().Index, pLocal2->GetSlsdData().Index);
      }

      if (pLocal1->IsShortLong() != pLocal2->IsShortLong()) {
        SystemLog.Printf("\tScript local variable '%s' types do not match!", pName1->GetString().c_str(), pLocal1->GetSlsdData().Index, pLocal2->GetSlsdData().Index);
      }
    }
  ...
  while (Result) {
    pLocal1 = CompareInfo.pScript1->FindVariable(pName2->GetString().c_str());
    if (pLocal2 == NULL) SystemLog.Printf("\tScript2 local variable '%s' was not found in script1!", pName1->GetString().c_str());
...
bool ObCompareScripts (obscriptcompare_t& CompareInfo) {
  ...
  Result = ObCompareScriptDataPriv(CompareInfo);
